...
export default function Lottery(props) {
  ...
  const [percentaje, setPercentaje] = useState("Refresh");
  ...
  const getInfoContract = async () => {
    if (provider == null) {
      ...
      const tempercentaje= await contract.percentageWithdrawal(parseInt(temIdGame));
      console.log(tempercentaje.toString());
      setPercentaje(tempercentaje);
    ...
    for (let i = 0; i < Nwinners; i++) {
      PrizeWinners.push(
        i +
          1 +
          "º Premio: " +
          (Pool *(100-percentaje)/100 * PercentageWinners[Nwinners][i]).toFixed(3).toString()
      );
    }
    ...
    if (NTickets <= 0 && MaxTicketsPlayers - TotalTicketsUser > 0) {
      ...
      try {
        await contract.join(NTickets, {
          value: ethers.utils.parseEther((CostTicket * NTickets).toString()),
        });
        setSuccess(true);
        setNTickets(0);
      } catch (e) {
